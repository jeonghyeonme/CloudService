...
### 1. AI 분석 및 데이터 정합성 (AI Analysis & Sync)
- [x] **한글 깨짐 이슈 수정**: Bedrock Claude 3 멀티모달 및 스캔 PDF 폴백 로직으로 해결.
- [x] **중복 요청 방지**: `aiSubmit.js` 내 DynamoDB 락킹 기반 중복 분석 차단 로직 구현.
- [ ] **채팅-리소스 DB 동기화**: 채팅창으로 업로드된 리소스 메타데이터의 '리소스 허브' 자동 등록 보완.
- [ ] **파일 공유 형식 통일**: 채팅창 직접 업로드 시에도 '파일을 공유했습니다: [파일명]' 형식으로 메시지 내용 통일.

### 2. 실시간 소통 및 채팅 UX (Real-time & Chat UX)
- [x] **입력 상태 알림 (`typing`)**: WebSocket 핸들러 및 프론트엔드 UI 연동 완료.
- [x] **검색 결과 스크롤 이동**: 메시지 검색 결과 클릭 시 해당 위치로 자동 스크롤 및 포커스 처리 구현 완료.

### 3. 유저 프로필 및 개인화 (User Profile)
- [x] **프로필 관리 기능**: 닉네임 수정 및 S3 기반 프로필 이미지 변경 API/UI 구현 완료.
- [ ] **프로필 실시간 반영**: 프로필 수정 시 서버 내 접속자들에게 웹소켓 브로드캐스트 전송 로직 추가.

### 4. 프로젝트 운영 및 인프라 (DevOps)
- [ ] **배포 자동화 고도화**: CloudFront 캐시 무효화 및 환경 변수 관리 최적화.
